...
let [remaining, setRemaining]=useState(10);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      navigate('/');
      toast.success("Successfully redirected to Home");

    }, 10000); // ⏱ 10 seconds
    return () =>{ clearTimeout(timer);clearInterval(time);}
  }, [navigate]);



  return (
    <div className={`animate-fadeIn transition-all delay-3000 ease-in-out  h-screen flex flex-col items-center justify-center bg-cover bg-no-repeat bg-center  text-center px-4 bg-bg`} > 
      <h1 className=" justify-center w-full text-6xl md:text-8xl font-extrabold tracking-wider  text-secondary-fg flex gap-2 items-center">
      ...
      <p className="mt-4 text-lg text-muted md:text-xl  text-muted-fg">
      ...
      <Button 
        onClick={() => navigate('/')}
        className="mt-10 text-secondary-fg bg-primary outline-1  outline-gray-500 "
        size="lg"
      >
        <House /> Go to Home
      </Button>
